...
enum Command {
    ...
    New {
        #[structopt(short, long, help = "content")]
        clip: String,
        #[structopt(short, long, help = "password")]
...
fn run(opt: Opt) -> Result<(), Box<dyn Error>> {
    match opt.command {
        Command::Get {
            ...
            let clip = get_clip(&opt.addr, req, opt.api_key)?;
            println!("{:#?}", clip);
            Ok(())
        }
        Command::New {
            ...
            let clip = new_clip(&opt.addr, req, opt.api_key)?;
            println!("{:#?}", clip);
        ...
        Command::Update {
            ...
            let original_clip = get_clip(&opt.addr, svc_req, opt.api_key.clone())?;
            ...
            let updated_clip = update_clip(&opt.addr, svc_req, opt.api_key)?;
            println!("{:#?}", updated_clip);
