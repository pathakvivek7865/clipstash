...
#[derive(Debug, thiserror::Error, Serialize, Responder)]
...
impl From<ServiceError> for ApiError {
    fn from(value: ServiceError) -> Self {
        match value {
            ServiceError::Clip(clip_err) => {
                Self::User(Json(format!("clip parsing error: {}", clip_err)))
            }
...
pub async fn new_api_key(database: &State<AppDatabase>) -> Result<Json<&str>, ApiError> {
    ...
    Ok(Json("Api key generated. See logs for detail."))
}
...
pub mod catcher {
    ...
    #[catch(500)]
    fn internal_err(req: &Request) -> Json<&'static str> {
